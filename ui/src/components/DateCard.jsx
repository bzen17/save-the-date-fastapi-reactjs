...
import { getCurrMonth, getFullDate, getMonthIndex, getYear } from "../utils/getDateAttr";
...
const DateCard = ({savedDates,scrollRef}) => {
    const countDays = (day,month) => {
      ...
      if (month === currMonth) {
        ...
        const diffinDays = Math.floor(Math.abs(date - new Date()) / (1000 * 60 * 60 * 24));
        return diffinDays;
      }
    }
    ...
    return (
        <Stack ref={scrollRef}
        className="scrollableContainer" direction="row" spacing={3}>
        {savedDates.map(el=> {
        return (
        <div style={{backgroundColor:'transparent',padding:'0.2rem', border: "2px solid #0D21A1", borderRadius:'1.75rem'}}>
        <Card
            key={el.id}
            sx={{
                minWidth: 250,
          ...
          >
            ...
            <Divider sx={{ borderColor: "primary.main", borderBottomWidth: 'medium' }} />
            <CardContent
              sx={{
            ...
            >
              in <b style={{fontSize:'1.5rem'}}>{countDays(el.day,el.month)}</b> days
            </Typography>
            </CardContent>
        </Card>
        </div>)})}
